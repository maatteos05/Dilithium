...
void AddNTT(int len, int32_t u[len], const int32_t v[len], const int32_t w[len]) {
  for (int i = 0; i < len; i++) {
    u[i] = fqred((int64_t)v[i] + (int64_t)w[i]);
  }
}

void MultiplyNTT(int len, int32_t c[len], const int32_t a[len], const int32_t b[len]) {
  for (int i = 0; i < len; i++) {
